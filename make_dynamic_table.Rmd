---
title: "Pharmacokinetic-pharmacodynamic models for time-courses of antibiotic effects – An exhaustive review"
output: 
  html_document:
...
If you like what you see and use this work, please cite our review paper, the 
full citation will be added after publication. In the mean time please cite it 
as : \ 

Minichmayr IK, Aranzana-Climent V, Friberg LE, Pharmacokinetic-pharmacodynamic 
models for time-courses of antibiotic effects – An exhaustive review, In 
manuscript, 2021. \ 
...
attached github repository https://github.com/Vincent-AC/pmx-models-atb-review \ 


Table last updated : 15th November 2021 \ 
...
2. Pathogens characteristics :
    +  Pathogen(s) : Full name of the pathogens included in the model
    +  Number of strains : How many strains were used to produce the data included in the model
3. Growth model characteristics :
    +  Growth model : Broad family of growth model can be one of : 1,2 or 3 metabolic states, growing and resting bacteria, 2 state life-cycle model, other.
    +  Growth equation : What is the equation characterizing the growth rate
    +  Delay in growth : Is there any delay in the growth rate equation ?
    +  Comments : Any additional information
4. Effect model characteristics : 
    +  Effect equation : Is the equation linking drug concentrations to effect a linear, linear with power, Emax or sigmoid Emax model ?
    +  Effect on death or growth : Is the drug effect applied as a reduction of growth or an increase in death rate ?
    +  Delay in effect : Is there a delay component in the effect equation ?
    +  Drug : Name of the drug providing the effect
    +  Pathogen : Full name of the pathogen on which the effect is applied
    +  Comments : Any additional information
5. Resistance model characteristics :
    +  Resistance model : What type of resistance model is included in the paper can be one of : Adaptive resistance, Subpopulations, Other
    +  Comments : Any additional information
6. Interaction model characteristics :
    +  Paradigm : Under which general paradigm can we categorise the interacation model, can be one of : Additivity, Loewe additivity, Bliss independance, Greco URSA model, Other
    +  Interaction effect : How does the interaction effect is parametrized ?
    +  Perpetrator : Which drug affect the effect of the other drug ? (concept from Wicha et al.)
    +  Victim : Which drug's effect is affected by the other drug ? (concept from Wicha et al.)
    +  Comments : Any additional information
7. PK data characteristics :
    +  Patient population : From which patient population were the PK parameters used in the paper derived ?
    +  Used for ? : Was the PK parameters used for setting up the system (+g dynamic experiments) or for simulations of expected bacterial densities ?
    +  Comments : Any additional information
...
single_drug_data <-
  read_xlsx(here("tables/single_drug.xlsx"))

combination_drug_data <-
  read_xlsx(here("tables/combination_drugs.xlsx"))
...
pk_data_summary <- pk_data_table %>% 
  ...
  distinct() %>%
  mutate(pk_population = case_when(pk_population == "no pk population" ~ NA_character_,
                                   TRUE ~ pk_population)) %>%
...
growth_model_summary <- growth_model_table %>%
  unite("growth_model",growth_model,growth_model_equation, growth_model_delay,growth_model_comments,sep=", ",na.rm=T) %>% 
...
resistance_model_summary <- resistance_model_table %>%
  ...
  mutate(n_row = paste0("resistance_",row_number()),
         resistance_model = str_to_sentence(resistance_model)) %>%
  ungroup() %>%
  pivot_wider(names_from = n_row,
...
interaction_model_summary <- interaction_model_table %>%
  ...
  mutate(
    ...
    interaction_effect = str_to_sentence(interaction_effect)
  ) %>%
  ungroup() %>%
  pivot_wider(names_from = n_row,
...
summary_table <- article_info_table %>%
  mutate(
    ...
    `Comments` = str_to_sentence(general_comments)
  ) %>%
...
l.summary_table_data_table_main <-
  split(summary_table_data_table %>% mutate(Link = paste0('<a  target=_blank href=https://pubmed.ncbi.nlm.nih.gov/', pmid, '/>Link</a>' ) ) %>% relocate(Link, .after = Article), summary_table_data_table$pmid)
...
l.studied_drugs <-
  split(
    ...
    ) %>% select(-pmid) %>% mutate(Drug = str_to_sentence(drug_name)) %>% rename(`Drug class` = group) %>% select(-drug_name, `Drug`, `Drug class`),
    studied_drugs_without_duplicate$pmid
  )
l.studied_pathogens <-
  split(
    studied_pathogens_table %>% select(-pmid) %>% rename(`Pathogen(s)` = pathogen_name,
                                                         `Number of strains` = n_strains),
    studied_pathogens_table$pmid
  )
l.growth_model <-
  split(
    growth_model_table %>% select(-pmid) %>% mutate_all(str_to_sentence) %>% rename(
...
l.effect_model <-
  split(
    effect_model_for_dynamic_table %>% select(-pmid) %>% mutate(
      effect_death_growth = str_to_sentence(effect_death_growth),
      effect_drug = str_to_sentence(effect_drug)
    ) %>% 
      rename("Effect equation" = effect_equation,
             ...
             "Comments" = effect_comments),
    effect_model_for_dynamic_table$pmid
  )
l.resistance_model <-
  split(resistance_model_table%>% select(-pmid) %>% mutate_all(str_to_sentence) %>% rename(`Resistance model` = resistance_model,
...
l.interaction_model <-
  split(interaction_model_for_dynamic_table%>% select(-pmid) %>%
          mutate_all(str_to_sentence) %>% 
          rename("Paradigm" = interaction_paradigm,
...
l.pk_data <-
  split(pk_data_table%>% select(-pmid) %>% mutate_all(str_to_sentence) %>% rename("Patient population" = pk_population,"Used for ?" = pk_usage, "Comments" = pk_comments), pk_data_table$pmid)
...
var tableFive = document.createElement('table');
                      ...
                      myCaptionFive.innerHTML = 'Resistance model characteristics';
...
caption = htmltools::tags$caption(
        ...
        "Experiment types : S = Static time-kill, D = Dynamic time-kill, StepDil = Stepwise dilution time-kill, HF = Hollow fibre time-kill, A = Animal time-kill, O = Other"
    )
) %>%
  formatStyle(
    1,
...
articles_year_graph_data <- left_join(article_info_table,studied_drugs_table) %>% 
  ...
  mutate(year_group=cut(year,breaks=c(1963,seq(1991,2021,5)),include.lowest=T,
...
ggplot(articles_year_graph_data,
  ...
  geom_text(data = articles_year_graph_data_summary,
    aes(
        x = year_group,y=30, label = n_papers_per_year_group,fill=NULL),
...
ggsave("figures/figure_drug_family.png",
       width=20,
       height=15,
       units="cm")
...
ggsave("figures/figure_experiment_type.png",
       width=20,
       height=15,
       units="cm")
...
ggplot(interaction_paradigm_statistics_table,
  ...
  scale_y_continuous(name = "Number of papers",
                     breaks = seq(0,40,5),
                     limits = c(0,40))+
  scale_x_discrete(name = "Interaction paradigm")+
  theme(axis.text.x = element_text(angle=45,hjust=1))

ggsave("figures/figure_interaction_paradigm.png",
       width=20,
       height=15,
       units="cm")
...
ggsave("figures/figure_interaction_effect.png",
       width=20,
       height=15,
       units="cm")
